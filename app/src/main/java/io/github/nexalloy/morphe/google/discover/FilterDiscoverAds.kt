...
/**
 * Filters promoted ad cards from the Google Discover (AGSA) feed.
 *
 * Ported from discover-ads-filter's StreamSliceFilterHook.
 *
 * Strategy:
 *  1. Hook the method that returns the Discover stream's renderable item list.
 *  2. In the after-hook, remove any item whose stable key contains an ad cluster token
 *     (currently "feedads" — the content-cluster identifier Google uses for ad slots).
 *  3. A fast fingerprint of the list avoids re-filtering identical frames (perf optimisation
 *     retained from the original repo).
 *
 * Cache note: method resolution is handled automatically by NexAlloy's DexKitCacheBridge
 * (SharedPrefCache keyed by app-lastUpdateTime + module commit hash), so no DexKit scan
 * occurs on subsequent launches unless AGSA or the module has been updated.
 */
